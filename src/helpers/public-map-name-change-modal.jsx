...
const PublicMapNameChangeModal = ({ sharedClick, handler }) => {
  const [sharedMapName, setSharedMapName] = useState("");
  const { t } = useTranslation();
  const navigate = useNavigate();

  useEffect(() => {
    const sharedMapInput = document.querySelector(".shared-map-name");
    if (sharedMapInput) {
      sharedMapInput.value = localStorage.getItem('openedMapName');
    }
  ...
  const saveACopyMap = () => {
    ...
    if (newMapName !== '') {
      if (newMapName !== localStorage.getItem('openedMapName')) {
        ...
        SharedMapService.saveACopyOfMapForPublicMap(sharedAnonymusData, newMapName);
      } else {
        Utils.modalm().open({
          exitButtonText: t("exitMsgTxt"),
          title: t("warningSendMailMsgTxt"),
          bodyContent: t("publicMapCopyNotSameNameMsgTxt"),
          buttons: [
            {
              text: t("okMsgTxt"),
              class: 'button yellow-button confirm-button',
...
export default PublicMapNameChangeModal;
