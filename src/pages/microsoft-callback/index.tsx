...
import { AzureLoginResponse } from '../../types/account';
  ...
  useEffect(() => {
    ...
    const handleMicrosoftCallback = async () => {
      try {
        ...
        if (code) {
          // Code ile token'ı API'ye gönder
          const loginResponse = await accountService.azureLogin(code);
          const data = loginResponse.data as AzureLoginResponse;
          ...
          if (data && data.result) {
            if (data.token) {
              ...
              if (data.email) {
                localStorage.setItem("microsoftEmail", data.email);
          ...
          console.error('Microsoft auth code not found in URL');
          navigate('/login');
        }
      } catch (error) {
